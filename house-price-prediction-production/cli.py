...
DATA_DIR = op.join(HERE, "data")  # Path to the data folder
...
def _run_job(cli_ctx, job_id, num_workers, num_threads_per_worker):
    ...
    with mlflow.start_run(
        run_name="cli-function", experiment_id=experiment_id, nested=True
    ) as parent_run:
        ...
        for job_spec in job_catalog["jobs"]:
            ...
            # Create a child run for each job
            with mlflow.start_run(
                run_name=spec_job_id, experiment_id=experiment_id, nested=True
            ) as child_run:
                # Log parameters
                mlflow.log_param("job_id", spec_job_id)
                mlflow.log_param("num_workers", num_workers)
                mlflow.log_param("num_threads_per_worker", num_threads_per_worker)
